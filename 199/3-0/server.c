...
void stupid_recv(void *zsock) {
	...
	assert( 0 <= zmq_recvmsg(zsock, &message, 0) );
	assert(zmq_msg_size(&message) > 0);
	assert( 0 == zmq_getsockopt(zsock, ZMQ_RCVMORE, &more, &more_size) );

	// check message size
	fprintf(stderr, "ERROR (see above): mp part [%.*s] (%p) received. more==%i.\n",
...
void receive_mp(void *zsock) {
	...
	assert( 0 <= zmq_recvmsg(zsock, &id_message, 0) );
	assert( 0 == zmq_getsockopt(zsock, ZMQ_RCVMORE, &more, &more_size) );
	...
	assert( 0 <= zmq_recvmsg(zsock, &first_message, 0) );
	assert( 0 == zmq_getsockopt(zsock, ZMQ_RCVMORE, &more, &more_size) );
	assert(more);
